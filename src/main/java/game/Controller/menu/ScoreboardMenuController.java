...
public class ScoreboardMenuController implements Initializable {
    ...
    public void createScoreBoard(ArrayList<User> users){
        ...
        hBox.getChildren().add(new ImageView(new Image("/game/images/avatars/2.png")));
    ...
    public void addLabel (int index , User user , HBox hBox){
        ...
        if (user.getAvatarFilePath()==null || user.getAvatarFilePath().isEmpty())
            hBox.getChildren().add(new ImageView(new Image("/game/images/avatars/" + user.getAvatarNumber()+".png"))) ;
        else
            hBox.getChildren().add(new ImageView(new Image(user.getAvatarFilePath())));
