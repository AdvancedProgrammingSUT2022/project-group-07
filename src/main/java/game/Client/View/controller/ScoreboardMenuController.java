...
public class ScoreboardMenuController implements Initializable {
    ...
    public ArrayList<User> getUsers (){
        try {
            Main.getClientHandler().sendRequest(new ClientRequest(TypeOfRequest.GET_SCOREBOARD));
            ServerResponse serverResponse = Main.getClientHandler().getResponse();
            return serverResponse.getUsers();
    ...
    public void addLabel (int index , User user , HBox hBox){
        ...
        for (Label label : labels) {
            if (ClientDataController.getCurrentUser()!=null
                    && user.getUsername().equals(ClientDataController.getCurrentUser().getUsername()))
